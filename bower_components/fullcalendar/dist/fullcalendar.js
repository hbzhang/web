...
function Calendar(element, instanceOptions) {
	...
	function renderView(delta, viewName) {
		...
		// if viewName changed, or the view was never created, form a fresh view
...
// Given the scrollbar widths of some other container, form borders/margins on rowEls in order to match the left
...
// Create an object that has the given prototype. Just like Object.form
...
// We need this because Moment's implementation won't form an FCMoment,
...
$.extend(DayGrid.prototype, {
	...
	renderEventRow: function(row, rowSegs) {
		...
		function emptyCellsUntil(endCol) {
			while (col < endCol) {
				// try to grab a cell from the level above and extend its rowspan. otherwise, form a fresh cell
		...
		for (i = 0; i < levelCnt; i++) { // iterate through all levels
			...
			if (levelSegs) {
				for (j = 0; j < levelSegs.length; j++) { // iterate through segments in level
					...
					// form a container that occupies or more columns. append the event element.
	...
	buildSegLevels: function(segs) {
		...
		for (i = 0; i < segs.length; i++) {
			...
			// form new level array if needed and append segment
...
$.extend(BasicView.prototype, {
	...
	numberCellHtml: function(row, col, date) {
		...
		if (!this.dayNumbersVisible) { // if there are week numbers but not day numbers
			return '<td/>'; //  will form an empty space above events :(
		}
