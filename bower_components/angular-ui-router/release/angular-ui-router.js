...
function $TemplateFactory(  $http,   $templateCache,   $injector) {
   ...
   * that is defined is used to form the template:
   *
 ...
 * * `'/users/:id'` - Matches '/users/bob' or '/users/1234!!!' or even '/users/' but not '/users' or
 *   '/users/bob/details'. The second path segment will be captured as the parameter 'id'.
 * * `'/users/{id}'` - Same as the previous example, but using curly brace syntax.
 * * `'/users/{id:[^/]*}'` - Same as the previous example.
 * * `'/users/{id:[0-9a-fA-F]{1,8}}'` - Similar to the previous example, but only matches if the id
...
function UrlMatcher(pattern, config) {
  ...
  // Find all placeholders and form a compiled pattern, using either classic or curly syntax:
  //   '*' name
  //   ':' name
  //   '{' name '}'
  //   '{' name ':' regexp '}'
 ...
 * <pre>
 * new UrlMatcher('/users/{id}?q').concat('/details?date');
 * new UrlMatcher('/users/{id}/details?q&date');
 * </pre>
 *
 ...
 * @example
 * <pre>
 * new UrlMatcher('/users/{id}?q&r').exec('/users/bob', {
 *   x: '1', q: 'hello'
 * });
 ...
 * @example
 * <pre>
 * new UrlMatcher('/users/{id}?q').format({ id:'bob', q:'yes' });
 * // returns '/users/bob?q=yes'
 * </pre>
 *
...
function $UrlMatcherFactory() {
   ...
   *   url: "/{users:dbObject}",
   *   controller: function($scope, $stateParams) {
   *     // $stateParams.users will now be an object returned from
   *     // the Users service
   *   },
   *   // ...
   * });
...
function $UrlRouterProvider(   $locationProvider,   $urlMatcherFactory) {
   ...
   *   // otherwise will take care of routing the users to the
   *   // specified url
   ...
   * @param {string|object} handler The path you want to redirect your users to.
   */
   ...
   *     // UserService is an example service for managing users state
   ...
   *       // Once the users has logged in, sync the current URL
   *       // to the router:
   *       $urlRouter.sync();
   *     });
   *   });
...
function $StateProvider(   $urlRouterProvider,   $urlMatcherFactory) {
   ...
   * To form a parent/child state use a dot, e.g. "about.sales", "home.newest".
  ...
  function $get(   $rootScope,   $q,   $view,   $injector,   $resolve,   $stateParams,   $urlRouter) {
    ...
    // Handles the case where a state which is the target of a transition is not found, and the users
    ...
    $state.transitionTo = function transitionTo(to, toParams, options) {
      ...
      // though we form the locals object itself outside resolveState(), it is initially
 ...
 *     <a href ui-sref="app.users({users: 'bilbobaggins'})">@bilbobaggins</a>
 *   </li>
 * </ul>
 * </pre>
 *
 *
 * When the app state is "app.users" (or any children states), and contains the state parameter "users" with value "bilbobaggins",
 * the resulting HTML will appear as (note the 'active' class):
 * <pre>
 * <ul>
 *   <li ui-sref-active="active" class="item active">
 *     <a ui-sref="app.users({users: 'bilbobaggins'})" href="/users/bilbobaggins">@bilbobaggins</a>
 *   </li>
 * </ul>
 * </pre>
 *
