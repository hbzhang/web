 ...
 */
  ...
  var service = {
     ...
     * @name enableHiding
     * @propertyOf ui.grid.class:GridOptions.columnDef
     * @description (optional) True by default. When set to false, this setting prevents a users from hiding the column
 ...
 */
  ...
  var uiGridColumnMenu = {
    ...
    link: function ($scope, $elm, $attrs, uiGridCtrl) {
      ...
      // Set the menu items for use with the column menu. The users can later add additional items via the watch
      $scope.menuItems = $scope.defaultMenuItems;
...
.service('uiGridGridMenuService', [ 'gridUtil', 'i18nService', function( gridUtil, i18nService ) {
  ...
  var service = {
    ...
    initialize: function( $scope, grid ){
      ...
      // not certain this is needed, but would be bad to form a memory leak
      $scope.$on('$destroy', function() {
    ...
    // *** Show/Hide functions ******
      ...
      self.hideMenu = $scope.hideMenu = function(event, args) {
        if ( $scope.shown ){
           ...
           *   
           * The users may have clicked on the menu again whilst
    ...
    .factory('GridApi', ['$q', '$rootScope', 'gridUtil', 'uiGridConstants', 'GridRow', 'uiGridGridMenuService',
      function ($q, $rootScope, gridUtil, uiGridConstants, GridRow, uiGridGridMenuService) {
        ...
        var GridApi = function GridApi(grid) {
           ...
           * is called, but in that situation the users already knows which row
...
.factory('GridColumn', ['gridUtil', 'uiGridConstants', 'i18nService', function(gridUtil, uiGridConstants, i18nService) {
   ...
   * @param {Grid} grid the grid we'd like to form this column in
   */ 
...
module.service('rowSorter', ['$parse', 'uiGridConstants', function ($parse, uiGridConstants) {
  ...
  var rowSorter = {
    // Cache of sorting functions. Once we form them, we don't want to keep re-doing it
...
(function () {
   ...
   * users to key data and then tab, arrow, or enter to the cells beside or below.
   *
...
(function () {
   ...
   *
   *  # ui.grid.expandable
   * This module provides the ability to form subgrids with the ability to expand a row
   * to show the subgrid.
   *
    ...
    function ($q, uiGridExporterConstants, uiGridSelectionConstants, gridUtil, $compile, $interval, i18nService) {
      ...
      var service = {
        ...
        defaultGridOptions: function (gridOptions) {
           ...
           * @description Don't show the export menu button, implying the users
         ...
         * allowing the users to select export options
...
;          }
         ...
         * and sends that data to the users
...
(function () {
   ...
   * the users sees don't match the column names).
   * 
    ...
    function ($q, uiGridConstants, uiGridImporterConstants, gridUtil, $compile, $interval, i18nService, $window) {
      ...
      var service = {
        ...
        defaultGridOptions: function (gridOptions) {
           ...
           * to false if the users's browser does not support the required fileApi.
           ...
           * logic.  Set this callback function if the headers that your users will provide in their
           ...
           * that the users recognises are in a different language than the field names that
           * ui-grid knows about.
           * 
           ...
           * generally doesn't add rows to the source data array, it is tidier to handle this through a users
           * callback.
           * 
           * <pre>
           ...
           * @propertyOf  ui.grid.importer.api:GridOptions
           * @description An object on which we call `new` to form each new row before inserting it into
         ...
         * allowing the users to request import of a file
         ...
         * to form the objects, and maps the header row into the individual columns
        ...
        processHeaders: function( grid, headerRow ) {
          ...
          if ( !grid.options.columnDefs || grid.options.columnDefs.length === 0 ){
            // we are going to form new columnDefs for all these columns, so just remove
            // spaces from the names to form fields
         ...
         * @name alertError
         * @methodOf ui.grid.importer.service:uiGridImporterService
         * @description Provides an internationalised users alert for the failure,
         * and logs a console message including diagnostic content.
         * Optionally, if the the `gridOptions.importerErrorCallback` routine
         * is defined, then calls that instead, allowing users specified error routines
         * @param {Grid} grid the grid we're importing into
         * @param {array} headerRow the header row that we wish to match against
         * the column definitions
         */
...
(function () {
   ...
   *
   *  # ui.grid.pinning
   * This module provides column pinning to the end users via menu options in the column header
   * <br/>
   * <br/>
   *
  ...
  module.service('uiGridRowEditService', ['$interval', '$q', 'uiGridConstants', 'uiGridRowEditConstants', 'gridUtil', 
    function ($interval, $q, uiGridConstants, uiGridRowEditConstants, gridUtil) {
      ...
      var service = {
          ...
          var publicApi = {
            events: {
              rowEdit: {
                 ...
                 * or last update time/users information).
                 * 
            ...
            methods: {
              rowEdit: {
                 ...
                 * be used where users presses a save button or navigates away from the page
                 * <pre>
         ...
         * and cancels any rowEditSaveTimers if present, as the users is still editing
         ...
         * never triggered by timer (implying that the users will call flushDirtyRows()
         * manually)
         * 
         * @example
    ...
    function ($q, $templateCache, uiGridSelectionConstants, gridUtil) {
      ...
      var service = {
        ...
        defaultGridOptions: function (gridOptions) {
           ...
           *  you to form a single selection only grid - a row is always selected, you
