...
.factory('$mdUtil', ["$cacheFactory", "$document", "$timeout", "$q", "$window", "$mdConstant", function($cacheFactory, $document, $timeout, $q, $window, $mdConstant) {
  ...
  return Util = {
    ...
    clientRect: function(element, offsetParent, isOffsetRect) {
      ...
      // The users can ask for an offsetRect: a rect relative to the offsetParent,
...
// The state of the current and previous 'pointer' (users's hand)
...
.factory('$mdGesture', ["$$MdGestureHandler", "$$rAF", "$timeout", function($$MdGestureHandler, $$rAF, $timeout) {
  ...
  addHandler('hold', {
    options: {
      // If the users keeps his finger within the same <maxDistance> area for
...
function InkRippleService($window, $timeout) {
  ...
  function attach(scope, element, options) {
    ...
    /**
     * Handles users input start and stop events
     *
     */
...
// In memory storage of defined themes and color palettes (both loaded by CSS, and users specified)
var PALETTES;
var THEMES;
var themingProvider;
...
// A color in a theme will use these hues by default, if not specified by users.
...
function generateThemes($injector) {
  ...
  // The users specifies a 'default' contrast color as either light or dark,
 ...
 * @param {string=} ng-change Angular expression to be executed when input changes due to users interaction with the input element.
 ...
 * will be resolved only if the users clicks the confirm action on the dialog.
 *
 * @returns {obj} an `$mdDialogPreset` with the chainable configuration methods:
 *
 ...
 *     Default true.
 *   - `clickOutsideToClose` - `{boolean=}`: Whether the users can click outside the dialog to
 *     close it. Default true.
 *   - `escapeToClose` - `{boolean=}`: Whether the users can press escape to close the dialog.
 *     Default true.
...
function MdDialogProvider($$interimElementProvider) {
  ...
  function dialogDefaultOptions($timeout, $rootElement, $compile, $animate, $mdAria, $document,
    ...
    function onShow(scope, element, options) {
      // Incase the users provides a raw dom element, always wrap it in jqLite
 ...
 *
 * @description
 * Dividers group and separate content within lists and page layouts using strong visual and spatial distinctions. This divider is a thin rule, lightweight enough to not distract the users from content.
 *
  ...
  function postLink(scope, element, attrs, ctrl) {
    ...
    function getGridStyle(colCount, rowCount, gutter, rowMode, rowHeight) {
      ...
      switch(rowMode) {
        ...
        case 'fit':
          // noop, as the height is users set
          break;
      }
 ...
 *   <input type="text" ng-model="users.name">
 * </md-input-container>
 *
 * <md-input-container>
 *   <label>Description</label>
 *   <textarea ng-model="users.description"></textarea>
 * </md-input-container>
 *
 * </hljs>
 */
 ...
 *
* @description
 * The circular progress directive is used to make loading content in your app as delightful and painless as possible by minimizing the amount of visual change a users sees before they can view and interact with content.
 *
 * For operations where the percentage of the operation completed can be determined, use a determinate indicator. They give users a quick sense of how long an operation will take.
 *
 * For operations where the users is asked to wait a moment while something finishes up, and it’s not necessary to expose what's happening behind the scenes and how long it will take, use an indeterminate indicator.
 *
 ...
 *
 * @description
 * The linear progress directive is used to make loading content in your app as delightful and painless as possible by minimizing the amount of visual change a users sees before they can view and interact with content. Each operation should only be represented by one activity indicator—for example, one refresh operation should not display both a refresh bar and an activity circle.
 *
 * For operations where the percentage of the operation completed can be determined, use a determinate indicator. They give users a quick sense of how long an operation will take.
 *
 * For operations where the users is asked to wait a moment while something finishes up, and it’s not necessary to expose what's happening behind the scenes and how long it will take, use an indeterminate indicator.
 ...
 * force the users to tab through all the radio buttons, `<md-radio-group>`
...
function SelectDirective($mdSelect, $mdUtil, $mdTheming, $interpolate, $compile, $parse) {
  ...
  function compile(element, attr) {
    // The users is allowed to provide a label for the select as md-select-label child
    ...
    return function postLink(scope, element, attr, ctrls) {
      ...
      mdSelectCtrl.setLabelText = function(text) {
        if (customLabel) return; // Assume that users is handling it on their own
...
function SelectMenuDirective($parse, $mdUtil, $mdTheming) {
  ...
  function SelectMenuController($scope, $attrs, $element) {
    ...
    self.init = function(ngModel) {
      ...
      // If the users doesn't provide a trackBy, we automatically generate an id for every
      // value passed in
      } else {
    ...
    self.removeOption = function(hashKey) {
      ...
      // Don't deselect an option when it's removed - the users's ngModel should be allowed
 ...
 * @restrict E
 * @description
 * The `<md-slider>` component allows the users to choose from a range of
 * values.
 *
 * As per the [material design spec](http://www.google.com/design/spec/style/color.html#color-ui-color-application)
 ...
 * the `md-primary` class.
 *
 * It has two modes: 'normal' mode, where the users slides between a wide range
 * of values, and 'discrete' mode, where the users slides between only a few
 * select values.
 *
 * To enable discrete mode, add the `md-discrete` attribute to a slider,
 * and use the `step` attribute to change the distance between
 * values the users is allowed to pick.
 *
 * @usage
 * <h4>Normal Mode</h4>
 * <hljs lang="html">
 ...
 * @param {number=} step The distance between values the users is allowed to pick. Default 1.
 * @param {number=} min The minimum value the users is allowed to pick. Default 0.
 * @param {number=} max The maximum value the users is allowed to pick. Default 100.
 */
 ...
 *     when the users starts scrolling past the original element.
  ...
  return {
    ...
    compile: function(element, attr, transclude) {
      ...
      return function postLink(scope, element, attr) {
        ...
        // Transclude the users-given contents of the subheader
        ...
        // of the element, that will be 'stickied' as the users scrolls.
        transclude(scope, function(clone) {
 ...
 * the users scrolls down, and reveal itself as the users scrolls up.
 ...
 * at one fourth the rate at which the users scrolls down. Default 0.5.
 */
    ...
    link: function(scope, element, attr) {
      ...
      function setupScrollShrink() {
        ...
        function updateToolbarHeight() {
          ...
          //
          // As the users scrolls down, the content will be transformed up slowly
 ...
 *
 * A tooltip will activate when the users focuses, hovers over, or touches the parent.
 *
 * @usage
 * <hljs lang="html">
 ...
 * currently visible.
 * @param {number=} md-delay How many milliseconds to wait to show the tooltip after the users focuses, hovers, or touches the parent. Defaults to 400ms.
...
function MdTooltipDirective($timeout, $window, $$rAF, $document, $mdUtil, $mdTheming, $rootElement, $animate, $q) {
  ...
  function postLink(scope, element, attr, contentCtrl) {
    ...
    function positionTooltip() {
      ...
      // If the users provided a direction, just nudge the tooltip onto the screen
...
(function () {
  ...
  function MdAutocompleteCtrl ($scope, $element, $q, $mdUtil, $mdConstant) {
    ...
    //-- This is to allow the users to scroll the list without causing it to hide
...
(function () {
   ...
   * This component allows you to provide real-time suggestions as the users types in the input area.
   *
...
function TabPaginationDirective($mdConstant, $window, $$rAF, $$q, $timeout, $mdMedia) {
  ...
  function postLink(scope, element, attr, tabsCtrl) {
    ...
    // Called when page is changed by a users action (click)
 ...
 * be initiated via data binding changes, programmatic invocation, or users gestures.
 *
...
function MdTabDirective($mdInkRipple, $compile, $mdUtil, $mdConstant, $timeout) {
  ...
  function compile(element, attr) {
    ...
    return function postLink(scope, element, attr, ctrls) {
      ...
      //defaultClickListener isn't applied if the users provides an ngClick expression.
 ...
 *
 * Initially, tabs will have an inherent size.  This size will either be defined by how much space is needed to accommodate their text or set by the users through CSS.  Calculations will be based on this size.
 *
